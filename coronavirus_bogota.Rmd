...
server <- function(input, output, session) {
    output$plot <- renderPlotly({
        animation <-
          ...
          geom_text(aes(y = casos_acum, label = as.character(casos_acum)), nudge_y = 50, size = 4) +
          geom_text(aes(x = 24, y = -180, label = paste("FECHA:", as.character(Fecha.de.diagnóstico))), size = 6) +
          labs(x = NULL,
