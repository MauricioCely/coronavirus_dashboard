...
Datos_BOG <- read.csv(textConnection(Datos_BOG))
...
localidades <- readOGR("Localidades/Loca.shp", verbose = FALSE)
...
```{r animated_barchart, fig.align="center"}
confirmados_serie <-
  Datos_BOG %>% filter(Ciudad.de.residencia == "Bogotá") %>%
  mutate(Estado                  = ifelse(Estado == "Recuperado" | Estado =="Fallecido", yes = Estado, no = "Activo"),
         Localidad.de.residencia = str_to_title(Localidad.de.residencia),
...
animation <-
  ...
  scale_y_continuous(breaks = seq(0,2000, 200), limits = c(-400, 1400), expand = c(0,0)) +
  coord_flip(clip = "off") +
  theme_ipsum() +
...
options(gganimate.dev_args = list(res = 150))
...
plot_gg(edad_localidad, height_aes = "fill", multicore = TRUE, raytrace = F, scale = 300, triangulate = T,
        width = 7, height = 7, offset_edges = T, windowsize = c(700, 750))
...
* Visualización - Los paquetes [plotly](https://plot.ly/r/), [ggplot2](https://ggplot2.tidyverse.org/), [rayshader](https://www.rayshader.com/) y [gganimate](https://gganimate.com/) para los gráficos y animaciones y el paquete [leaflet](https://rstudio.github.io/leaflet/) para el mapa.
